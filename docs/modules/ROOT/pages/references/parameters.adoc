...
[CAUTION]
====
This parameter exposes an alpha feature of Espejote.
The Admission CRD is subject to change at any moment.
====
...
The namespaced Admission resources to deploy.

The keys are parsed as namespaced names `<namespace>/<name>` and used as names and namespaces of the admission.
If no namespace is provided the admission is created in the fallback namespace provided in the `namespace` parameter.

=== `admissions.<name>.metadata` / `admissions.<name>.spec`
  ...
  my-namespace/my-admission: <1>
    ...
    spec: <3>
      ...
      template: |
        ...
        admission.allowed('Always allow')
      webhookConfiguration:
        rules:
          - apiGroups: ['']
            ...
            scope: Namespaced
----
<1> The key is parsed as a namespaced admission `<namespace>/<name>`.
<2> The metadata of the admission (optional).
<3> The spec of the admission (required).

The metadata and spec of the admission.
...
====
This parameter exposes an alpha feature of Espejote.
The ClusterAdmission CRD is subject to change at any moment.
====
...
The ClusterAdmission resources to deploy.

=== `clusterAdmissions.<name>.metadata` / `clusterAdmissions.<name>.spec`
      ...
      webhookConfiguration:
        rules:
          - apiGroups: ['']
            ...
            scope: '*'
----
<1> The name of the cluster admission.
<2> The metadata of the cluster admission (optional).
<3> The spec of the cluster admission (required).

The metadata and spec of the cluster admission.
