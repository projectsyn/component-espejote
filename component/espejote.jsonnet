...
local espejote = com.Kustomization(
  ...
  kustomization+: {
    // NOTE(sg): We extend `kustomization.resources` with
    // `kustomization.resources` of the `contrib` Kustomization object to
    // render the `default` and `contrib` Kustomizations provided by Espejote
    // in a single call to `kustomize`.
